...
double runTSEDA(string XmlFile, string RscAlcFile,double& SchTime, int& iteration) {
    ...
    InitProModelOfTskSch(PMS, NumOfAncestors, NumOfNonDescendants, STS);
    ...
    while ((double) (clock() - start) / CLOCKS_PER_SEC < Parameter_TSEDA.RunTimeRatioOfStg1 * SchTime) {
        ...
        for (int i = 0; i < comConst.NumOfTsk; ++i) {
            for (int j = 0; j < comConst.NumOfRsc; ++j) {
                ...
                PMR[i][j] = (1 - Parameter_TSEDA.theta1) * PMR[i][j] +
                            Parameter_TSEDA.theta1 * (1.0 * count / Parameter_TSEDA.NumOfEliteOfPop);
            }
        }
        ...
        for (int i = 0; i < comConst.NumOfTsk; ++i) {
            for (int j = 0; j < comConst.NumOfTsk; ++j) {
                ...
                PMS[j][i] = (1 - Parameter_TSEDA.theta2) * PMS[j][i] +
                            Parameter_TSEDA.theta2 * (1.0 * count / Parameter_TSEDA.NumOfEliteOfPop);
            }
        }
        ++iteration;
        NewPopulation.clear();
